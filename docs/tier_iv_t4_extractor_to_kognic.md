...
conversion:
  ...
  # auth: ~/.config/kognic/credentials.json  # optional; or [client_id, client_secret];
  #                                          # omit -> credentials from the environment
  projects:
    - project_external_id: my_cuboid_project
      batch_external_id: my_cuboid_batch # optional; omit -> latest open batch
...
| `auth`                            | No       | `None`  | Credentials passed to `KognicIOClient(auth=...)`: either a path to a credentials JSON file, or a `[client_id, client_secret]` pair. Omit to use the credentials in the environment (`KOGNIC_CREDENTIALS`, or `KOGNIC_CLIENT_ID` + `KOGNIC_CLIENT_SECRET`) — preferred, so secrets stay out of the config file.                                                                        |
...
flowchart TD
  ...
  Q -- "scene_external_id" --> Scene["download_scene()<br/>resolve external_id → scene_uuid"]
  Q -- "scene_id" --> SceneU["download_scene()<br/>use scene_uuid directly"]
  Q -- "neither" --> All["download_all()<br/>get_project_annotations<br/>(annotation_type, batch)"]
  Scene --> T{"annotation_type set?"}
  SceneU --> T
  T -- "no" --> AllTypes["get_annotations_for_scene<br/>(all annotation types)"]
  T -- "yes" --> OneType["get_project_annotations<br/>(annotation_type, batch)<br/>filtered to scene_uuid"]
  AllTypes --> Out["<output_base>/<project_external_id>/<scene_external_id | scene_id>.json"]
  OneType --> Out
...
conversion:
  ...
  # Single-scene download — set ONE of the two below:
