...
class DeepenToT4Converter(AbstractConverter[DeepenToT4ConverterOutput]):
    ...
    def _find_start_end_time(self, t4_dataset_dir):
        t4_dataset = Tier4(data_root=t4_dataset_dir, verbose=False)
        end_t4_dataset_timestamp = 0
        for frame_index, sample in enumerate(t4_dataset.sample):
            if frame_index == 0:
                self._start_sec = (
                    misc_utils.nusc_timestamp_to_unix_timestamp(sample.timestamp) - 2.0
                )
            if sample.timestamp > end_t4_dataset_timestamp:
                end_t4_dataset_timestamp = sample.timestamp
        self._end_sec = misc_utils.nusc_timestamp_to_unix_timestamp(end_t4_dataset_timestamp) + 2.0
