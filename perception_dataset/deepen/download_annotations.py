...
def get_annotation_labels(dataset_ids: List[str], dataset_dir: str, output_name: str) -> None:
	...
	datasets_url = (
		f"https://tools.deepen.ai/api/v2/clients/{CLIENT_ID}/labels_of_dataset_ids?labelSetId=default_semantic"
	)
...
def get_paint3d_labels(dataset_ids: List[str], dataset_dir: str, output_name: str) -> None:
	...
	Example of a response:
	{
		...
		'paint-metadata': '{"format": "pako_compressed", "paint_categories": ["car", "wall"], 
	...
	for dataset_id in dataset_ids:
		datasets_url = (
			f"https://tools.deepen.ai/api/v2/datasets/{DATASET_ID}/label_types/3d_point/paint_labels?stageId=QA&sensor_id=lidar&labelSetId=default"
		)
		try:
		...
		lidarseg_category_indices = np.frombuffer(
			decompressed_data,
			dtype=np.uint8
		)
		...
		for file_id, frame_size in enumerate(paint_metadata['frame_sizes']):
			...
			with open(lidarseg_anno_filename, "wb") as f:
				lidarseg_category_indices[previous_frame_size: previous_frame_size+frame_size].tofile(f)
			
			previous_frame_size = frame_size
			lidarseg_annos_info.append(
				annos_info
			)
