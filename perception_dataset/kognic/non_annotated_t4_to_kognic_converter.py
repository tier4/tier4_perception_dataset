...
class NonAnnotatedT4ToKognicConverter(AbstractConverter[None]):
    ...
    def __init__(
        ...
        # Kept for compatibility with the convert.py/config interface. The
        # Tier IV extractor exports every selected T4 sample frame and does not
        # resample by annotation_hz.
    ...
    def _convert_one_scene(self, input_dir: Path | str, output_dir: Path | str) -> None:
        ...
        if self._lidar_channels and not (seq_path / "data" / "LIDAR_CONCAT_INFO").is_dir():
            raise FileNotFoundError(
                f"Required directory is missing for LiDAR extraction: "
                f"{seq_path / 'data' / 'LIDAR_CONCAT_INFO'}"
            )
    ...
    def _extract_ego_poses(self) -> Dict[str, KognicModel.EgoVehiclePose]:
        ...
        for sample in self._samples:
            ...
            sample_by_channel = self._sample_data_by_sample_and_channel.get(sample_token, {})
            sample_data = sample_by_channel.get(_LIDAR_CONCAT_CHANNEL)
            if sample_data is None:
                sample_entries = self._sample_data_by_sample.get(sample_token, [])
                sample_data = sample_entries[0] if sample_entries else None
    ...
    def _collect_image_copies(
        ...
        for sample in self._samples:
            sample_data = self._sample_data_by_sample_and_channel.get(sample["token"], {}).get(
                camera_channel
            )
            if sample_data is None:
                if self._drop_camera_token_not_found:
                    logger.warning(
                        f"Camera {camera_channel} missing for sample {sample['token']}; skipping"
                    )
                continue
    ...
    def _extract_pointclouds(self, seq_path: Path, out_dir: Path, lidar_channel: str) -> None:
        ...
        count = 0
        for sample in self._samples:
            concat_sample_data = self._sample_data_by_sample_and_channel.get(
                sample["token"], {}
            ).get(_LIDAR_CONCAT_CHANNEL)
