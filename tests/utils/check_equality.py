...
def diff_check_annotation(target_dir: Path, source_dir: Path) -> None:
    ...
    assert target_annotation_dir.is_dir(), f"Target annotation directory not found: {target_annotation_dir}"
    assert source_annotation_dir.is_dir(), f"Source annotation directory not found: {source_annotation_dir}"
...
def diff_check_rosbag(source_input_bag_path: Path, target_input_bag_path: Path) -> None:
    ...
    assert len(source_db3_files) == 1, f"Expected 1 .db3 file in source, found {len(source_db3_files)}"
    ...
    assert target_metadata_yaml_path.is_file(), f"Target metadata not found: {target_metadata_yaml_path}"
    assert source_metadata_yaml_path.is_file(), f"Source metadata not found: {source_metadata_yaml_path}"
    ...
    assert target_metadata == source_metadata, (
        ...
        f"Metadata files are not equal"
    )
...
def diff_check_t4_dataset(target_dir: Path, source_dir: Path) -> None:
    ...
    assert source_has_input_bag == target_has_input_bag, (
